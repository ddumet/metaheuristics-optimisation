...
class genalg:
    ...
    def crossover(self):
        ...
        Crossover consists in selecting pairwise parents to generate children. It happens in
        several steps:
        1) Select effective parents according to a general parameter pc (crossover probability)
        ...
        for i in range(0, nb_o):
            ...
            os1 = np.append(self.parents[couple[0]][:c],self.parents[couple[1]][c:])
            offsprings = np.vstack((offsprings, os1))
    
            os2 = np.append(self.parents[couple[1]][:c],self.parents[couple[0]][c:])
    ...
    def mutate(self):
        ...
        Uniform mutation is used here (generating new genes within the feasible space) 
        ...
        for offspring in self.offsprings:
            ...
            offspring[mask] = np.random.uniform(self.lower, self.upper, (np.sum(mask)))
